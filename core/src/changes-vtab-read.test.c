...
static void testChangesQueryForTable() {
  ...
  assert(strcmp(query,
                ...
                "__crsql_site_id as site_id,      __crsql_opid as opid    FROM "
                "\"foo__crsql_clock\"") == 0);
  sqlite3_free(query);
...
static void testChangesUnionQuery() {
  ...
  assert(
      strcmp(query,
             "SELECT tbl, pks, cid, col_vrsn, db_vrsn, site_id, opid FROM "
             "(SELECT    "
             ...
             "db_vrsn,      __crsql_site_id as site_id,      __crsql_opid as "
             "opid    FROM "
             "\"bar__crsql_clock\")  ORDER BY opid ASC") == 0);
  sqlite3_free(query);
