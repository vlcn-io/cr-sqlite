...
static void testChangesUnionQuery() {
  ...
  assert(
      strcmp(
          query,
          "SELECT tbl, pks, cid, col_vrsn, db_vrsn, site_id, _rowid_, seq, cl "
          "FROM (SELECT\n"
          "          'foo' as tbl,\n"
          ...
          "      t1.\"a\" = t2.\"a\" AND t2.__crsql_col_name = '-1' LEFT JOIN "
          "__crsql_site_id as t3 ON t1.__crsql_site_id = t3.ordinal UNION ALL "
          "SELECT\n"
          "          'bar' as tbl,\n"
          ...
          "      t1.\"x\" = t2.\"x\" AND t2.__crsql_col_name = '-1' LEFT JOIN "
          "__crsql_site_id as t3 ON t1.__crsql_site_id = t3.ordinal) ") == 0);
  sqlite3_free(query);
  ...
  assert(
      strcmp(query,
             ...
             "JOIN __crsql_site_id as t3 ON t1.__crsql_site_id = t3.ordinal "
             "UNION ALL SELECT\n"
             "          'bar' as tbl,\n"
             "          crsql_pack_columns(t1.\"x\") as pks,\n"
             "          t1.__crsql_col_name as cid,\n"
