...
def test_c1_3_quoted_identifiers():
  ...
  check_clock = lambda t : c.execute("SELECT rowid, __crsql_version, __crsql_col_name, __crsql_site_id FROM {t}__crsql_clock".format(t=t)).fetchall()
...
def test_c1_c5_compound_primary_key():
  ...
  c.execute("SELECT a, b, __crsql_version, __crsql_col_name, __crsql_site_id FROM foo__crsql_clock").fetchall()
  # with pytest.raises(Exception) as e_info:
...
def test_c1_6_single_primary_key():
  ...
  c.execute("select crsql_as_crr('foo')")
  c.execute("SELECT a, __crsql_version, __crsql_col_name, __crsql_site_id FROM foo__crsql_clock").fetchall()
